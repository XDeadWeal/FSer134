package rtx.kimiko.utils.render.wave;
...
public final class WindWaveRenderer {
    ...
    private static void init() {
        if (solidPipeline == null || cutoutPipeline == null) {
            ...
            if (!RenderSystem.getDevice().precompilePipeline(renderPipeline).isValid() || !RenderSystem.getDevice().precompilePipeline(renderPipeline2).isValid()) {
                ...
                Kimiko.LOGGER.warn("WindWave disabled: terrain_wave shader failed to compile");
                return;
            }
        ...
        if (uniformBuffer == null || uniformBuffer.isClosed()) {
            uniformBuffer = RenderSystem.getDevice().createBuffer(() -> "kimiko_wave_params", 136, 16L);
        }
    }
    ...
    public static void beginFrame() {
        ...
        try {
            ...
            Kimiko.LOGGER.warn("WindWave disabled after error", throwable);
        }
    }
    ...
    private static void closeUniform() {
        if (uniformBuffer != null) {
            try {
                ...
                Kimiko.LOGGER.debug("[WindWave] Failed to close uniform buffer", throwable);
            }
            uniformBuffer = null;
        }
    }
