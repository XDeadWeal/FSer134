package rtx.kimiko.utils.render.warmup;
...
public class Load {
    ...
    public static void warmupFonts() {
        ...
        for (String string4 : stringArray = new String[]{"montserrat-medium", "montserrat-regular", "montserrat-semibold", "montserrat-bold", "sf", "sf-medium", "kimiko", "mainmenu"}) {
            try {
                Render2D.textWidth(string4, string3, 8.0f);
            }
            catch (Throwable throwable) {
                Kimiko.LOGGER.warn("[Load] textWidth warmup failed for font {}: {}", string4, throwable.getMessage());
            }
            try {
                Render2D.msdfWidth(string4, string3, 8.0f);
            }
            catch (Throwable throwable) {
                Kimiko.LOGGER.warn("[Load] msdfWidth warmup failed for font {}: {}", string4, throwable.getMessage());
            }
        }
    }
    ...
    public static void runStartupWarmup() {
        ...
        try {
            ...
            Kimiko.LOGGER.warn("[Load] CustomPetWarmup.warmup failed", throwable);
        }
    }
}
